/*
 * jquery.layout 1.3.0 - Release Candidate 22
 *
 * Copyright (c) 2010 
 ...
 *
 * $Date: 2010-01-26 08:00:00 -0800 (Tue, 26 Jan 2010) $
 * $Rev: 3022 $
 * 
			...
			if ($P && $C && $P.is(":visible")) { // if No Content OR Pane not visible, then skip
				...
				function setOffsets () {
					$.swap( $C[0], { height: "auto", display: "block", visibility: "hidden" }, function(){
						s.above = eC.offsetTop;
						s.below = $P.innerHeight() - eC.offsetTop - eC.offsetHeight;
					});
				};
				...
				setOuterHeight($C, $P.innerHeight() - s.above - s.below, true); // true=autoHide
			}
		});
	};
